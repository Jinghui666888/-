...
class SBAG(SBAG_trainer):
    ...
    def __init__(self, config):
        ...
        self.Wcm = [nn.Parameter(torch.FloatTensor(embeding_size, embeding_size).to(self.device)) for _ in range(self.n_heads)]
        self.Wam = [nn.Parameter(torch.FloatTensor(embeding_size, embeding_size).to(self.device)) for _ in range(self.n_heads)]
        self.scale = torch.sqrt(torch.FloatTensor([embeding_size])).to(self.device)

        self.W1 = nn.Parameter(torch.FloatTensor(embeding_size * self.n_heads, embeding_size).to(self.device))
        self.W2 = nn.Parameter(torch.FloatTensor(embeding_size * self.n_heads, embeding_size).to(self.device))
        self.W3 = nn.Parameter(torch.FloatTensor(embeding_size, embeding_size).to(self.device))
        ...
        with open("/Volumes/halis没有报销/dataset/twitter15/dev.pkl", "rb") as f:
    ...
    def publisher_encoder(self, X_user, X_user_id, user_node_feat):
        ...
        A_us = torch.FloatTensor(A_us).to(self.device)
    ...
    def edge_infer(self, x, edge_index):
        ...
        xi_weight = self.cls(x_i).softmax(dim=1)[:, 1]
        xj_weight = self.cls(x_j).softmax(dim=1)[:, 1]
    ...
    def retweet_user_encoder(self, X_ruser, X_ruser_id, user_node_feat, test_flag):
        ...
        edge_index = self.A_uu.edge_index.to(self.device)
...
class SBAG_weibo(SBAG_trainer):
    ...
    def __init__(self, config):
        ...
        self.max_poolings = nn.ModuleList(
            [nn.MaxPool1d(kernel_size=config['maxlen'] - K + 1) for K in config['kernel_sizes']])
        ...
        self.W2 = nn.Parameter(torch.FloatTensor(embeding_size * self.n_heads, embeding_size).to(self.device))
        self.W3 = nn.Parameter(torch.FloatTensor(embeding_size, embeding_size).to(self.device))
        self.linear_fu = nn.Linear(400, 1).to(self.device)
    ...
    def forward(self, X_source_wid, X_source_id, X_user_id, X_ruser_id, test_flag=False):
        ...
        user_node = self.A_uu.node_feat.to(self.device)
        user_node_feat = torch.tanh(self.linear(user_node))
