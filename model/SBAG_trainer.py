...
class SBAG_trainer(nn.Module):
    ...
    def fit(self, X_train_source_wid, X_train_source_id, X_train_user_id, X_train_ruid, y_train, y_train_cred, y_train_rucred,
        ...
        self.optimizer = torch.optim.Adam(
            ...
            weight_decay=self.config['reg'], amsgrad=True
        )
        ...
        for epoch in range(1, self.config['epochs'] + 1):
            ...
            for i, data in enumerate(dataloader):
                X_source_wid, X_source_id, X_user_id, X_ruid, batch_y, batch_y_cred, batch_y_rucred = [
                    item.to(self.device) for item in data
                ]
                ...
                print('Batch[{}] - loss: {:.6f}  acc: {:.4f}%({}/{})'.format(
                    i, loss.item(), accuracy, corrects, batch_y.size(0)
                ))
            ...
            if epoch > self.config['epochs'] // 2 and self.patience > 2:
                ...
                self.load_state_dict(torch.load(self.config['save_path'], map_location=self.device))
                now_lr = self.adjust_learning_rate(self.optimizer)
                print("Adjusted learning rate:", now_lr)
                self.patience = 0
    ...
    def evaluate(self, X_dev_source_wid, X_dev_source_id, X_dev_user_id, X_dev_ruid, y_dev, epoch):
        y_pred = self.predict(X_dev_source_wid, X_dev_source_id, X_dev_user_id, X_dev_ruid, task=None)
    ...
    def predict(self, X_dev_source_wid, X_dev_source_id, X_dev_user_id, X_dev_ruid, task, test_flag=False):
        ...
        with torch.no_grad():
            for i, data in enumerate(dataloader):
                X_source_wid, X_source_id, X_user_id, X_ruid = [
                    item.to(self.device) for item in data
                ]
